/* La Parepa C2.13 - cuadros históricos exactos + recuperación JSON + paginación remota */
(function (global) {
  'use strict';
  ...
  function startSalesListener() {
    ...
    state.salesUnsubscribe = salesQuery(signature).limit(PAGE_SIZE).onSnapshot(snapshot => {
      ...
      console.error('[C2.13] Error escuchando las primeras 10 ventas del día operativo:', error);
  ...
  function offsetDayKey(day, offset) {
    ...
    const d = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2] + Number(offset || 0)));
    return `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, '0')}-${String(d.getUTCDate()).padStart(2, '0')}`;
  }

  function currentWeekRange(day) {
    const parts = String(day || '').split('-').map(Number);
    const d = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2]));
    const isoDay = d.getUTCDay() || 7; // lunes=1 ... domingo=7
    return {
      start: offsetDayKey(day, 1 - isoDay),
      end: offsetDayKey(day, 7 - isoDay)
    };
  }

  function currentMonthRange(day) {
  ...
  function mergeRemoteAndPendingSales(remoteSales, rangeStart, rangeEnd) {
    const pending = (global.obtenerVentasPendientesSync?.() || [])
      .map(item => item?.venta)
      .filter(Boolean)
      .map(normalizeSale)
      .filter(v => String(v?.diaClave || '') >= rangeStart && String(v?.diaClave || '') <= rangeEnd);
    const localPending = (global.obtenerVentasStorage?.() || [])
      .map(normalizeSale)
      .filter(v => String(v?._syncEstado || '').toLowerCase() === 'pendiente')
      .filter(v => String(v?.diaClave || '') >= rangeStart && String(v?.diaClave || '') <= rangeEnd);
    const map = new Map();
    [...remoteSales, ...localPending, ...pending].forEach((sale, index) => {
      const key = sale?._localId || sale?._docId || `${sale?.diaClave || ''}|${sale?.comanda ?? sale?.recibo ?? index}`;
      map.set(key, sale);
    });
    return [...map.values()];
  }

  function periodSummary(sales) {
    const normalized = (sales || []).map(normalizeSale);
    const active = normalized.filter(v => !isCancelled(v));
    const cancelled = normalized.filter(isCancelled);
    const deliveries = active.filter(isDelivery);
  ...
  async function loadCurrentSummaryCards(force = false) {
    // IMPORTANTE: estos tres cuadros NO dependen de las tablas/resúmenes históricos.
    // Se calculan directamente desde TODAS las ventas del rango actual en Firestore.
    // Esto evita que "Hoy", "Esta semana" o "Este mes" queden desactualizados.
    const today = salesSignature(); // respeta el mismo día operativo del POS/caja (corte 12:30 a. m.)
    ...
    let allSales = [];
    try {
      // Una sola consulta cubre semana + mes. diaClave YYYY-MM-DD permite rango lexicográfico exacto.
      const snapshot = await db.collection('ventas')
        .where('diaClave', '>=', rangeStart)
        .where('diaClave', '<=', rangeEnd)
        .get();
      const remoteSales = snapshot.docs.map(normalizeSale);
      allSales = mergeRemoteAndPendingSales(remoteSales, rangeStart, rangeEnd);
    } catch (error) {
      console.warn('[C2.13] No se pudieron consultar los cuadros históricos directamente; usando datos locales disponibles.', error);
      allSales = mergeRemoteAndPendingSales(
        (global.obtenerVentasStorage?.() || []).map(normalizeSale),
        rangeStart,
        rangeEnd
      );
    }

    const daySales = allSales.filter(v => v?.diaClave === today);
    const weekSales = allSales.filter(v => String(v?.diaClave || '') >= week.start && String(v?.diaClave || '') <= week.end);
    const monthSales = allSales.filter(v => String(v?.diaClave || '') >= month.start && String(v?.diaClave || '') <= month.end);

    const day = periodSummary(daySales);
    const weekSummary = periodSummary(weekSales);
    const monthSummary = periodSummary(monthSales);

    const set = (id, value) => { const el = document.getElementById(id); if (el) el.textContent = value; };
    const apply = (prefix, row) => {
      set(`${prefix}Ventas`, `${Number(row?.ventas || 0)} venta(s)${Number(row?.canceladas || 0) ? ` · ${Number(row.canceladas)} cancelada(s)` : ''}`);
      set(`${prefix}Total`, `${core.money(row?.total || 0)} · ${Number(row?.domicilios || 0)} domicilio(s)`);
    };

    apply('histHoy', day);
    apply('histSemana', weekSummary);
    apply('histMes', monthSummary);

    return {
      day: { ...day, diaClave: today },
      week: { ...weekSummary, inicio: week.start, fin: week.end },
      month: { ...monthSummary, inicio: month.start, fin: month.end },
      documentosConsiderados: allSales.length
    };
  }
